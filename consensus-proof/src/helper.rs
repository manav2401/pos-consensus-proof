...
pub fn verify_signature(
    ...
    Ok(expected_signer.eq(&recovered_signer))
}
...
pub fn verify_tx_data(
    tx_data: &str,
    expected_hash: &str,
) -> Result<heimdall_types::MilestoneMsg, Box<dyn Error>> {
    // Decode the transaction data
    let mut decoded_tx_data = BASE64_STANDARD.decode(tx_data)?;

    // Calculate the hash of decoded data
    let tx_hash = sha256(decoded_tx_data.as_slice());

    // Typecast the expected tx hash
    let expected_hash_bytes = TxHash::from_hex(expected_hash)?;

    if !expected_hash_bytes.eq(&tx_hash) {
        return Err("transaction hash mismatch".into());
    }

    // Deserialize the message to extract the milestone bytes
    let decoded_message = deserialize_msg(&mut decoded_tx_data)?;

    Ok(decoded_message.msg.unwrap())
}
...
pub fn verify_precommit(
    mut precommit_message: Vec<u8>,
    expected_hash: &str,
) -> Result<bool, Box<dyn Error>> {
    // Decode the precommit message
    let precommit = deserialize_precommit(&mut precommit_message)?;
