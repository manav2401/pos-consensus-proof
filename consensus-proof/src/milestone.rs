...
impl MilestoneProver {
    ...
    pub fn prove(&self) {
        ...
        let milestone_result = verify_tx_data(&self.inputs.tx_data, &self.inputs.tx_hash);
        if milestone_result.is_err() {
            panic!(
                "Unable to verify transaction data: {:?}",
                milestone_result.err()
            );
        }

        let milestone = milestone_result.ok().unwrap();
        ...
        for (i, sig) in self.inputs.sigs.iter().enumerate() {
            let decoded_precommit_message = match hex::decode(self.inputs.precommits[i].as_str()) {
                Ok(v) => v,
                Err(e) => {
                    panic!("Error decoding precommit message: {}", e);
                }
            };
            ...
            match verify_precommit(decoded_precommit_message.clone(), &self.inputs.tx_hash) {
                Ok(valid) => {
                    if !valid {
                        panic!(
                            "Precommit message verification failed for signer: {}",
                            self.inputs.signers[i].as_str()
                        );
                    }
                }
                Err(e) => {
